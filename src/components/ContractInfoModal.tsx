...
const ContractInfoModal: React.FC<ContractInfoModalProps> = ({
  ...
  const explorerBaseUrl = selectedNetwork === 'mainnet'
    ? 'https://block.voi.network/explorer'
    : 'https://testnet.block.voi.network/explorer';

  const contracts = {
    mainnet: {
      // Replace with actual mainnet addresses when available
      vnsRegistry: 'TBD',
      vnsResolver: 'TBD',
      vnsRegistrar: 'TBD',
      usdc: 'TBD',
  ...
  const currentContracts = selectedNetwork === 'mainnet' ? contracts.mainnet : contracts.testnet;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography variant="h6" component="h2" sx={{ mb: 2, color: '#8B5CF6' }}>
          About enVoi
        </Typography>

        <Typography variant="body1" sx={{ mb: 2 }}>
          enVoi is a decentralized naming service built on the Voi blockchain.
          It allows users to register and manage human-readable names that map to their Voi addresses,
          making it easier to send and receive assets on the network.
        </Typography>
        ...
        <Typography variant="body2" sx={{ mb: 2 }}>
          Network: <strong>{selectedNetwork === 'mainnet' ? 'Voi Mainnet' : 'Voi Testnet'}</strong>
        </Typography>

        <List sx={{ mb: 2 }}>
          <ListItem sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', py: 0.5 }}>
